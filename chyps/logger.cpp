...
void StartLogger(const int a_rank_id, const int a_world_size,
  ...
  if (a_world_size > 32) {
    if (a_rank_id == 0) {
      std::cout << "Too many processes " << a_world_size << " in use for logger"
                << std::endl;
      std::cout << "Logger being DISABLED." << std::endl;
      std::cout
          << "For optimal performance, recompile with CHYPS_LOGGER_OFF defined."
          << std::endl;
      std::cout << "NOTE: This is not a steadfast requirement, however using "
  ...
  std::string number = std::to_string(a_rank_id);
  assert(world_digits >= static_cast<int>(number.length()));
  std::string id_suffix =
      std::string(world_digits - number.length(), '0') + number;
  std::string log_name = std::string("log_") + id_suffix + std::string(".txt");
  try {
