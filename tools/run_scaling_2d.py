...
# Base case is 9 Elements
# Elements at refined level calculated as 9*(2^(2*t)),
...
strong_scaling_num_elem = 589824
...
weak_scaling_num_elem_start = 9216
weak_scaling_growth = 7 # Cores at level is weak_starting_cores*(2**((t+3)*2))
print("Planned weak scaling study:\n")
print("NumElem".center(10)+"Cores".center(10)+"Nodes".center(10)+"Elem/Core".center(10))
for t in range(0,weak_scaling_growth):
  elem = weak_scaling_num_elem_start*(2**(2*t))
  cores = int(weak_starting_cores*(2**(t*2)))
  nodes = int(math.floor(cores/cores_per_node))
  if cores - nodes*cores_per_node > 0:
      nodes = nodes + 1
  print(str(elem).center(10)+ \
...
for t in range(1,strong_scaling_growth):
  dir = "simdir"+str(t)
  os.mkdir(dir)
  os.chdir("./"+dir)
  execute_command = "../../scale_test -m ../../periodic-square.mesh -rt 8 -o 2" # This is 589824 Elements
  r = open("run","w")
  r.write("#!/bin/bash \n")
